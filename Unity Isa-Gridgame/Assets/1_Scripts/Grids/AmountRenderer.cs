...
public class AmountRenderer : BaseClass
{
        ...
        for (int y = 0; y < MainGrid.Height; y++)
        {
            for (int x = 0; x < MainGrid.Width; x++)
            {
                if (MainGrid.GetTile(new Vector2Int(x, y)).id != ID.none)
                {
                    ...
                    //Map values to uv
                    uv[verticesAndUvIndex + 1] = new Vector2(uv00[currentAmount].x, uv00[currentAmount].y);
                    uv[verticesAndUvIndex + 2] = new Vector2(uv11[currentAmount].x, uv00[currentAmount].y);
                    uv[verticesAndUvIndex + 0] = new Vector2(uv00[currentAmount].x, uv11[currentAmount].y);
                    uv[verticesAndUvIndex + 3] = new Vector2(uv11[currentAmount].x, uv11[currentAmount].y);
                }
                else
                {
                    //Remove quads
