...
public class TempRenderer : BaseClass
{
    ...
    private void GenerateQuads()
    {
        ...
        for (int y = 0; y < MainGrid.Height; y++)
        {
            ...
            {
                if (MainGrid.GetTile(new Vector2Int(x, y)).id != ID.none)
                {
                    int currentTemp = MainGrid.GetTile(new Vector2Int(x, y)).temp;
                    int currentTileTemp;
                    if (currentTemp > maxTempRange)
                    {
                        currentTileTemp = TileAmount - 1;
                    }
                    else if (currentTemp < minTempRange)
                    {
                        currentTileTemp = 0;
                    }
                    else
                    {
                        currentTileTemp = (int)math.remap(minTempRange, maxTempRange, 0, TileAmount - 1, currentTemp);
                    }
                    ...
                    //Map values to uv
                    uv[verticesAndUvIndex + 1] = new Vector2(uv00[currentTileTemp].x, uv00[currentTileTemp].y);
                    uv[verticesAndUvIndex + 2] = new Vector2(uv11[currentTileTemp].x, uv00[currentTileTemp].y);
                    uv[verticesAndUvIndex + 0] = new Vector2(uv00[currentTileTemp].x, uv11[currentTileTemp].y);
                    uv[verticesAndUvIndex + 3] = new Vector2(uv11[currentTileTemp].x, uv11[currentTileTemp].y);
                }
                else
                {
                    //Remove quads
    ...
    {
        int quadAmount = MainGrid.Width * MainGrid.Height;
